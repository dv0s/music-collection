...
<div class="flex flex-col flex-wrap w-full">
    <h1 class="text-2xl text-gray-800 mb-6">Album aanpassen</h1>

    <form action="{{ route('album-store') }}" method="POST" class="w-full" id="album">
        @csrf

        <div class="flex flex-row flex-wrap">
            <section class="w-1/2 flex flex-col flex-wrap">
                <x-form-input type="text" name="title" id="title" label="Album titel" :value="old('title')"></x-form-input>
                <x-form-input type="date" name="released_at" id="released_at" label="Samengekomen op" :value="old('released_at')" ></x-form-input>
